...
-- 1-1
-- Эта таблица конечно только для того, чтобы показать связь 1-1.
DROP TABLE IF EXISTS photos_accessibility;
CREATE TABLE photos_accessibility (
	photo_id SERIAL PRIMARY KEY,
	photo_id_enlarged_copy BIGINT UNSIGNED DEFAULT NULL, -- Вот здесь я не уверен, можно ли так делать.
	alt varchar(255) DEFAULT NULL,

    FOREIGN KEY (photo_id) REFERENCES photos(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (photo_id_enlarged_copy) REFERENCES photos(id) ON UPDATE CASCADE ON DELETE set NULL
);
...
CREATE TABLE presents(
    ...
    FOREIGN KEY (sender_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE notifications(
	id SERIAL PRIMARY KEY,
	notification_type_id BIGINT UNSIGNED,
	user_id BIGINT UNSIGNED NOT NULL,
	body text,
    ...
    FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (notification_type_id) REFERENCES notification_types(id) ON UPDATE CASCADE ON DELETE CASCADE
);
